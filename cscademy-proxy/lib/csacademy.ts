...
export async function login(email: string, password: string): Promise<Session> {
  ...
  // Step 3: Get user info + workspace from /workspace/ page
  ...
  // GET /workspace/ with XHR returns JSON with state.Workspace[]
  try {
    const wsPageRes = await fetchRetry(
      ...
      { label: "GET workspace page", retries: 2 }
    );
    if (wsPageRes.ok) {
      const wsPageData = await wsPageRes.json();
      const workspaces = wsPageData?.state?.Workspace || [];
      console.log(`[CSA]   Found ${workspaces.length} workspace(s)`);
      if (workspaces.length > 0) {
        const ws = workspaces[0];
        workspaceId = String(ws.id || "");
        userId = String(ws.userId || "");
        console.log(`[CSA]   Using workspace: id=${workspaceId} name="${ws.name}" userId=${userId}`);
      }
    }
  } catch (e: any) {
